...
export default function KepalaSectiPage() {
    ...
    try {
      ...
      if (data && typeof data === 'object' && 'data' in data) {
        ...
        const filteredData = Array.isArray(data.data)
          ? data.data.filter((b: Berkas) => b.status === 'DI_KEPALA_SEKSI')
          : [];
        ...
        let filteredData = allData.filter((b: Berkas) => b.status === 'DI_KEPALA_SEKSI');
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-3xl font-bold text-gray-900">👔 Kepala Seksi</h1>
      </div>
              ...
              berkasList.map((berkas, index) => (
                <tr key={berkas.id} className="hover:bg-blue-50 transition-colors duration-150">
                  ...
                  <td className="px-4 py-4 whitespace-nowrap" style={{ width: '15%' }}>
                    <Link href={`/berkas/proses/kepala-seksi/${berkas.id}`}>
                      <Button
                        size="sm"
                        variant="outline"
                        className="text-[10px] px-2 py-1 text-indigo-600 hover:bg-indigo-100 hover:text-indigo-700 border-indigo-300 font-medium"
                      >
                        Tinjau
                      </Button>
                    </Link>
